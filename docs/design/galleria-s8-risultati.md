...
2. **L'avvio non si aggiusta da solo su un file già gonfio**: 2,1 s dei 2,7 s stanno in `open`, cioè nelle due scansioni che il firmware fa dentro la prima syscall persist. Non è aggirabile dall'app: il file contiene i record morti delle 688 scritture della chiave 2 e delle foto sostituite, e il firmware compatta solo oltre ~615 KB fisici (mai raggiunti qui).
