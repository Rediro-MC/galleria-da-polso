...
> **Nota (17/09/2026): quasi tutto chiuso altrove.** Le voci qui sotto portano l'esito fra parentesi
> quadre; resta aperto solo ciò che si decide con l'orologio in mano (**O11**, Pebble 2 Duo reale:
> `galleria-s8-hardware.md` §9), più una svista del dev server che non tocca l'orologio.

- **[chiuso — D28, 05/09/2026]** **Heap layout B < 40 KB su emery** (38,4–40,0 KB secondo il font):
  **accettato dall'utente** (`apps/galleria/PIANO.md` §6, D28: si accettano 38,7–40 KB). Le opzioni valutate allora erano: accettare (nessuna allocazione a regime oltre la strip A da 7,3 KB
  ...
  `__attribute__((packed))`). Nessuna delle altre è stata applicata.
- **[in parte chiuso — R13/S9-prep; resta O11]** **Flint, stile trasparente**: l'anello da 1 px sul dithering Floyd–Steinberg è al limite della leggibilità (f01); R = 2 chiuderebbe le
  ...
  e documentare che lo stile trasparente è pensato per il Pebble Time 2. **Fatto (R13, S9-prep)**: la config
  page mostra `#styleFlintHelp` — «Sul Pebble 2 Duo il contorno è sottile: con foto molto dettagliate scegli
  lo stile pieno» — solo su flint e solo con lo stile trasparente (`page.js:233`). Il giudizio sul vetro vero
  resta da fare (**O11**).
- **[chiuso — P5/S9-prep, condizione rivista in UX-2/D98]** **Anton in stile trasparente, taglia A**: controforma dello `0` di 6 px → 2 px con l'anello (28 % aperta): le cifre tendono a chiudersi; in B
  (9 px) va bene. Francois One e Staatliches sono i font consigliati per lo stile trasparente: **suggerito nella pagina**
  (`#s_style_hint`, «Con questo stile si leggono meglio Francois One e Staatliches», mostrato con stile a
  contorno — 1 o 2 — **e** font 0–2, `page.js:230`).
- **[accettato]** **Staatliches «1» = asta di 11 px**: nella griglia a celle fisse resta spazio ai lati («I0:44»). Accettato (carattere del font).
- **[aperto, senza conseguenze sull'orologio]** Il dev server accetta `digit_style` 2/3 anche per scenario flint (la normalizzazione D26 sta nella pagina, non nel server).
- **[accettato]** **Ombra che sporge di 1 px nel margine interno del vicino** (solo stili 3D, riga 12 h, cifre larghe adiacenti: il passo di
  D25 v3 copre riempimento + anello, non l'ombra, profonda S = 2 su emery): **accettato** — l'ombra cade sull'anello del
  glifo successivo, disegnato dopo, quindi si vede al più 1 px di rosso-opposto contro l'alone. Su flint non esiste (S = 0, D26).
