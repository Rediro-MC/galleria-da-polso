...
> Comando usato: `gh issue create -R coredevices/PebbleOS --title "<titolo>" --body-file <(sed -n '/^<!-- BODY-START -->/,/^<!-- BODY-END -->/p' docs/design/galleria-s9-issue-pebbleos.md | sed '1d;$d')`. **Modificata il 20/09/2026 (13:05 UTC)** con
> `gh issue edit 2106 -R coredevices/PebbleOS --body-file <(stesso filtro)`: due frasi corrette e la riga «_Edited 2026-09-20 …_» in
> fondo (§«Verifica del 20/09/2026», ultimo punto).
...
- **Correzioni dopo la pubblicazione (20/09/2026, pomeriggio: `/code-review` sul commit `81d890a`)**: il corpo pubblicato diceva
  ancora «plus a few hundred rewrites of a 4-byte state record, one per wrist shake» — la correzione del punto «688» qui sopra non
  era arrivata al testo inglese: `model.c` al commit `9afe0f0` (riga 221) scriveva il record **solo in `deinit`** e solo se cambiato —
  e «after six doublings» per i 512 KiB: da `PERSIST_STORAGE_INITIAL_ALLOC` = 4 KiB (`persist/service.c:26`, `prv_grow` raddoppia)
  sono **sette** (4→8→16→32→64→128→256→512). Corretti nel corpo qui sotto e **online il 20/09/2026 alle 13:05 UTC con
  `gh issue edit 2106 --body-file …`**, su scelta dell'utente fra quattro opzioni, con una riga finale in corsivo «_Edited 2026-09-20:
  corrected two details … Nothing else changed._» (badge «edited» di GitHub; 0 commenti prima e dopo). Il blocco qui sotto è di nuovo
  **identico al corpo online** (verificato con `gh issue view` subito dopo la modifica). Le correzioni sono state trovate dal
  `/code-review` sul commit `81d890a` e verificate da un workflow di 50 agenti prima della modifica online.
...
_Edited 2026-09-20: corrected two details in "Current Behavior" item 2 and "Why" item 6. The old 4-byte state record was rewritten on app exit when it had changed, not once per wrist shake; reaching the 512 KiB step from the 4 KiB initial allocation takes seven doublings, not six. Nothing else changed._
<!-- BODY-END -->
