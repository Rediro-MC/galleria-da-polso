...
Orologio reale (serve il telefono: da Linux non c'è BLE): nell'app Core Devices/Pebble Settings → Connectivity → «Use LAN developer
connection» ON e, nella scheda dell'orologio, ⋯ → «Dev Connection» ON (porta 9000); poi `pebble ping|install|logs|screenshot --phone <IP>`
(`--phone` senza IP = CloudPebble). Alternative: `--adb` (wireless debugging) e `--cloudpebble` (`pebble login`). Comandi e insidie in
`apps/galleria/CLAUDE.md` §Comandi S8, runbook `docs/design/galleria-s8-runbook-android.md`.
...
8. Persist: `persist_exists()` prima di leggere; schema versionato + CRC; chunk poi lunghezza; scritture con debounce (≤ 1/min); in `deinit` solo il flush delle impostazioni pendenti, mai altre scritture persist (revisione perf 04/09: ogni chiave nuova è una scansione lineare del file); `persist_get_max_size()` con fallback 4096; ≤ 75% della quota; gestire `E_OUT_OF_STORAGE`.
...
- Screenshot da conservare: in `docs/design/<app>/` (un set per gate), ogni PNG citato per nome in un `.md`; gli altri restano fuori
  dal repo (`apps/*/*.png` è in `.gitignore`).
