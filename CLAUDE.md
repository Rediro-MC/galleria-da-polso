...
- **Modelli per grado di importanza (regola permanente, richiesta dall'utente il 27/08/2026):** ogni sessione/lavoro va diviso in compiti classificati **bassa / media / alta** importanza; i compiti di importanza **bassa e media** si delegano ad agenti/workflow con modello **Opus**, quelli di importanza **alta** (codice C sull'orologio, integrazione, decisioni di design, revisioni adversariali) a **Fable** (o li tiene l'orchestratore Fable). Annotare la classificazione nel `PIANO.md` dell'app (esito della sessione).
