# `i18n/` — dizionari della config page (S10/D35; S11: es e pt; S12: anteprima; UX-1: lessico unico; UX-2: struttura; UX-3: flusso della foto; UX-4: una stringa fr)
...
Oggi: **135 chiavi × 6 lingue**, 39.834 B (`wc -c i18n/messages.json`, 14/09/2026, fine UX-4 —
**una sola stringa ritoccata**, `preview_stale` in francese, D133: +14 B; erano 39.820 B a fine
...
Testo per lingua: it 3.944 car., en 3.846, de 4.372, fr 4.496, es 4.014, pt 4.083 (la voce più lunga
...
glossario di `../../../docs/design/galleria-s10-i18n.md` §3 copre **tutte e 135** le chiavi con i
...
restano a 29 e 16 e `tools/build_i18n.py` non cambia (D123).
...
array **nell'ordine del file**. `src/pkjs/i18n.js` (**36.500 B** misurati il 14/09/2026 a fine UX-4;
36.486 B a fine UX-3, 34.859 B a fine UX-2, 33.589 B a fine UX-1, 34.031 B con 135 chiavi dopo S12,
30.124 prima di S12)
finisce nello stato dell'hash della pagina (tutte e sei le lingue: il cambio lingua nella pagina è
istantaneo, senza tornare al PKJS; i sei dizionari pesano **36.934 caratteri** di base64url
— 27.700 B di JSON UTF-8 × 4/3 —, erano 36.915 a fine UX-3, 35.024 a fine UX-2, 33.684 a fine UX-1,
≈ 33,4 k con le 135 chiavi di S12, ≈ 30,0 k con 121 e ≈ 19,9 k con quattro lingue);
`test/fixture_i18n.js` è la stessa cosa per i test node (stessi 36.500 B).
...
- **Le `<option>` sono minuscole** (D68): maiuscola iniziale solo se il testo comincia con un nome
  proprio o con «Ora»/«Font» («Ora in alto, info sotto», «Font di sistema (solo Ora in alto)»);
  ...
  «nur Kontur», «Kontur mit Schatten», «Systemschrift (nur Uhrzeit oben)» —, quindi in tedesco
...
- **Tripwire di lunghezza** (D70, in `tools/build_i18n.py`, attiva sia in generazione sia in
  ...
  **lista esplicita di chiavi**, misurato in code point sul testo **renderizzato** (segnaposto
  ...
  - `OPTIONS`, le **29** chiavi che finiscono in una `<option>`: **≤ 28**, con tre eccezioni a **36**
    ...
    `opt_minutes`/`opt_hours` resi con «60» e «3»;
