...
>
> **Sera del 19/09/2026**: il `PATCH` di `PUBLISH.md` §0.1 è stato **eseguito** (descrizione online = il testo
> «Beta 0.4.0», **794 caratteri**; titolo «Galleria»); `emery_screenshot_6.png` (foto **CC-BY-SA-4.0**, mai
> online) è stato **tolto** su decisione dell'utente: **8 screenshot in `store/`** (5 emery + 3 flint),
> **tutti online**.
>
> **Stato al 20/09/2026**: la **1.0.0 è pubblicata** dalle **00:14 locali del 20/09/2026** (API
> `published_date` 2026-09-19T22:14 UTC; `pebble publish --non-interactive --no-gif-all-platforms --version
> 1.0.0 --release-notes …` con `store/release_notes_1.0.0.txt`, log locale `publish_100.log`; commit `dd628c0`
> + tag `v1.0.0`) e la **descrizione online è quella di §2** = `store/description.txt` (**827 caratteri**:
> «Three layouts», «or shake for the next one», «Version 1.0.0», niente «Beta»), mandata lo stesso giorno
> con un secondo `PATCH` di `PUBLISH.md` §0.1 (HTTP 200). **Titolo «Galleria» e 8 screenshot invariati**
> (`make_assets.py --check` verde prima del publish, niente rigenerazione). Questo documento è stato
> riallineato alla 1.0.0 il **20/09/2026**: §1, §2, §3.S14, §5, §6 e §7.

## 1. Identità dell'app

| Campo | Valore | Fonte |
|---|---|---|
| Nome nello store | **Galleria** dalla **0.4.0** (decisione **D42** di S11, 06/09/2026: si rinomina con il `PATCH` di `PUBLISH.md` §0/§0.1, `--form-string "title=Galleria"`). **Online «Galleria»** (verificato sull'API pubblica il 19/09/2026: rinominata **dall'utente dalla dashboard** fra il 18/09 e il 19/09, non con il `PATCH`, che è stato eseguito **due volte per la sola descrizione**: la sera del **19/09/2026** e il **20/09/2026** con la 1.0.0, `PUBLISH.md` §0.1). Il nome era stato messo da `--name` alla creazione | spec S9 §2 e `galleria-s11-lingue-es-pt.md` D42; `--name` è efficace **solo alla creazione** (`publish.py:806`, `:814`; `PUBLISH.md`, campo *name* e §9), quindi il nome si cambia solo via API/dashboard (`PUBLISH.md` §0) |
...
| Versione | **1.0.0** — **pubblicata il 20/09/2026** (00:14 locali; API `published_date` 2026-09-19T22:14 UTC; commit `dd628c0`, tag `v1.0.0`) — S14: «Ora in basso», intervalli 6 h e 12 h, contorno di contrasto dal 15 %, «Ottimizza» spuntata di serie, via le frecce del font (D136–D141). Già pubblicate: la **0.4.0** il **18/09/2026** (tag `v0.4.0`; beta: spagnolo e portoghese di S11 **più** la pagina delle impostazioni rifatta — S12 anteprima, UX-1/UX-2/UX-3); la **0.1.0** (prima release, beta, decisione **U7**, tag `v0.1.0-beta`) e la **0.2.0** (multilingua en/it/de/fr, tag `v0.2.0`), queste due il 05/09/2026. La **0.3.0 non è mai stata pubblicata** (D126 di UX-4): ne resta solo il testo delle release notes in §3.1 — il file `store/release_notes_0.3.0.txt` è uscito dal repo il 17/09/2026 e il `.pbw` storico non è più in `build_s8/` | `package.json` → `version` = `1.0.0` (commit `dd628c0`, 20/09/2026; 0.4.0 dal 14/09, 0.3.0 solo nel repo dal 06/09, 0.1.0 il 05/09; il `1.0.0` di prima del 05/09 veniva dal commit iniziale `9afe0f0` del 30/08/2026 ed è stato portato a 0.1.0 da `87a4562`) |
...
> **Riscritta il 20/09/2026 per la 1.0.0** (commit `dd628c0`): **827 caratteri** ASCII (`wc -m store/description.txt` = 828 con il newline finale), **online dal 20/09/2026** con il `PATCH` di `PUBLISH.md` §0.1 (HTTP 200) — **oltre** gli **800** chiesti dall'utente per la 0.4.0, che lo store ha comunque accettato (il suo tetto reale non è noto, riquadro qui sotto). Rispetto al testo della 0.4.0 (794 caratteri, online dalla sera del 19/09/2026) cambiano tre cose, tutte della **1.0.0**, +33 caratteri in tutto: (1) «Two layouts: time with steps, battery and date, or time only» → «**Three layouts: time on top or at the bottom**, with steps, battery and date, or time only» (**S14/D136**); (2) «shake for the next one» → «**or** shake for the next one» (ritocco di forma); (3) «**Beta 0.4.0**» → «**Version 1.0.0**» (numero di versione scelto dall'utente il 20/09/2026); `diff` con 3 righe cambiate su 9. Il resto di questa nota è la storia del testo fino alla 0.4.0, **riscritto il 06/09/2026** (allora per la 0.3.0, che non è mai uscita; per D126 il testo era destinato alla **0.4.0**, uscita il 18/09/2026 **senza** quel testo, mandato poi la **sera del 19/09/2026**): allora **794 caratteri**, sotto il tetto di **800** chiesto dall'utente. Rispetto alla stesura corta del 05/09 (789 caratteri, online dalle 23:27 di quel giorno, `PUBLISH.md` §0; al 19/09/2026, prima del `PATCH`, online c'era quel testo senza il prefisso «Beta 0.2.0, »: **777 caratteri**) cambiano quattro cose, tutte volute: (1) «**Designed for Pebble Time 2 (colour display); also runs on Pebble 2 Duo**» nel primo capoverso al posto della riga piatta «Pebble Time 2 and Pebble 2 Duo» del terzo; (2) «Photo upload tested on Android **and iPhone**» (prova del 06/09, `galleria-s8-risultati.md` §S8b); (3) «Settings page in English, Italian, German, French, **Spanish and Portuguese**»; (4) «**Beta 0.4.0**» (era «Beta 0.3.0» fino al 14/09: D126, unica modifica di UX-4 al testo — *variante minima*, `diff` con la sola riga 9 cambiata). Per far posto: «Pick and crop them on your phone» → «Crop them on the phone», «a shake skips to the next one» → «shake for the next one», «4 styles (solid, transparent, 3D)» → «4 digit styles», «PebbleOS 4.32 or newer» → «PebbleOS 4.32+», «Remove Galleria and install it again» → «Remove and reinstall Galleria».
>
> ⚠️ La descrizione **non si aggiorna dalla CLI** (`PUBLISH.md` §9): si manda con il `PATCH` multipart di `PUBLISH.md` §0 — **lo stesso comando che porta `title=Galleria`** (D42; il nome è già online, verificato sull'API il 19/09/2026, §1) — oppure si incolla in dashboard (https://appstore-api.repebble.com/dashboard). La release **1.0.0** è uscita il **20/09/2026** (00:14 locali) e **dallo stesso giorno** online c'è il testo di §2 = `store/description.txt` (**827 caratteri**, secondo `PATCH` di `PUBLISH.md` §0.1 **eseguito**, HTTP 200); dalla sera del **19/09/2026** c'era quello della 0.4.0 (**794 caratteri**) e prima ancora quello della 0.2.0, 777 caratteri (nota di testa del documento e blockquote qui sopra).

Lo store **non è localizzato**: un solo testo in inglese (la riga italiana della prima stesura è
caduta con la riscrittura corta). **827 caratteri** (`store/description.txt`, testo della **1.0.0**
online dal 20/09/2026; erano 794 fino alla 0.4.0), **tutto ASCII**, 5 capoversi: foto e rotazione
...
> **Rebble** ("preparing a submission"), quindi indicativo. L'unico tetto mai fissato è quello chiesto
> dall'utente per la **0.4.0**: **800 caratteri**; il testo online della **1.0.0** (`store/description.txt`)
> ne usa **827** e lo store lo ha accettato (`PATCH` del **20/09/2026**, HTTP 200), quindi il tetto reale
> continua a non essere noto. Anche gli accenti sono evitati (il multipart va in UTF-8, ma il comportamento
> del server non è provato).

```text
Your photos as the watchface. Designed for Pebble Time 2 (colour display); also runs on Pebble 2 Duo. Crop them on the phone: up to 12 rotate on the watch, or shake for the next one.

Big bitmap clock: 6 fonts, 4 digit styles, white or black text from the photo. Three layouts: time on top or at the bottom, with steps, battery and date, or time only. 12 h or 24 h.
...
Version 1.0.0, open source (MIT): github.com/Rediro-MC/galleria-da-polso. Fonts: Anton, Bebas Neue, Barlow Condensed, Francois One, Staatliches (OFL). Demo photos: CC0.
```
...
byte-identici; tetto chiesto dall'utente per la **0.4.0**: **800 caratteri** — il testo della **1.0.0**
ne usa **827**: lo store non ha un tetto noto e ha accettato il `PATCH`).
Verificato il **20/09/2026**: il blocco qui sopra e `store/description.txt` sono byte-identici
(**828** caratteri con il newline finale, **827** senza; tutto ASCII, nessuna virgoletta doppia).

### Alternativa per l'utente: la variante «con anteprima» (798 caratteri)

> **Non scelta e oggi superata** (D126: con la 0.4.0 è andata in rete la *variante minima* — **mandata la sera
> del 19/09/2026** —, cioè il testo del 06/09 con il solo «Beta 0.3.0» → «Beta 0.4.0»; dal **20/09/2026**
> online c'è il testo della **1.0.0**, `store/description.txt` = §2, **827 caratteri**, che dice già «Three
> layouts: time on top or at the bottom» e «Version 1.0.0»). Il blocco qui sotto è fermo al **14/09/2026**:
> dell'idea resta solo la frase sull'anteprima, che nel testo in vigore continua a non esserci. Resta qui
> perché è l'unica alternativa preparata e l'utente può riprenderla senza rifare il lavoro: va però riportata
> **sul testo della 1.0.0** prima di incollarla in `store/description.txt` e nel blocco di §2, aggiungendo a
> §5 la riga di fonte indicata sotto.
...
> §2 = `store/description.txt` (**827 caratteri**, 1.0.0): i conti su 1.500/1.600 caratteri sono solo storia.
...
> ✅ **20/09/2026**: **1.0.0 pubblicata il 20/09/2026 alle 00:14 locali** con queste note (`store/release_notes_1.0.0.txt`, **505 caratteri** = 506 B con il newline finale, solo inglese), su richiesta dell'utente dopo la prova sul PT2 reale; commit `dd628c0` + tag `v1.0.0`; descrizione da **827 caratteri** online con il `PATCH` di `PUBLISH.md` §0.1. **Il record completo della release (comando, orari UTC, log, esito del `PATCH`, pre-check) è in `PUBLISH.md` in testa**, non qui. Il file è `store/release_notes_1.0.0.txt` e il blocco qui sotto ne è la copia byte-identica (verificato il 20/09/2026 con `diff`). La nota storica che segue racconta lo stato della bozza del 19/09.

> **Storia — la bozza del 19/09/2026**, superata dalla riga ✅ qui sopra: allora mancavano il **numero di
> versione** (**0.5.0 o 1.0.0**: l'utente ha scelto **1.0.0** il 20/09/2026, `package.json` → `version` =
> `1.0.0`) e la pubblicazione, fatta il **20/09/2026** su sua richiesta (§6, variante «nuova release»).
> Finché il numero non c'era la bozza teneva il segnaposto `X.Y.Z`: `store/release_notes_1.0.0.txt` è stato
> scritto e pubblicato quel giorno, e il blocco qui sotto ne è la copia. Nessuna riga per lingua: vale la
> regola del 18/09/2026 (solo inglese) in testa a §3.

```text
Galleria 1.0.0 - a new Clock at bottom layout, longer photo intervals, a clearer time on busy photos.
...
✅ **Fatto il 20/09/2026**: `pebble publish` aggiorna **solo** versione, note e `.pbw` (§6), quindi insieme alla
release è stato lanciato il `PATCH` di `PUBLISH.md` §0.1 (HTTP 200). Online ci sono gli **827 caratteri** di
`store/description.txt` (= §2): «**Three layouts: time on top or at the bottom, with steps, battery and date,
or time only**» e «**Version 1.0.0**» al posto di «Two layouts» e «Beta 0.4.0». La regola resta: ogni cambio
del testo della descrizione vuole quel `PATCH`, mai la CLI.
...
(oggi `src/c/ui_time.h:54-60`, contratto di `ui_time_set_sync_progress`, e `src/c/ui_time.c:837-841`, ramo `s_show_sync` con `prv_draw_sync_icon`: ricontrollati il 20/09/2026 dopo D136; nella build 0.1.0 la scritta era davvero «Foto k/n», da S10/D32 è icona + «k/n») e con **U8/R10** (applicato il 05/09/2026 dall'orchestratore)
...
`ui_time.c:NN`** della tabella erano quelli del 14/09 e **non** sono stati ricontrollati prima della **1.0.0**
(pubblicata il 20/09/2026, §3.S14).
✅ **20/09/2026**: riportate a oggi anche le righe **4, 5, 12, 23, 24 e 29**, verificate con `grep -n` per
identificatore — `footerLabels()` `page.js:535` (`btn_add_ok` `:539`, `btn_save` `:543`), `UNAVAIL`/`applyUnavailable()`
`:192`/`:194` (chiamate `:211-212`), `#s_style_hint`/`#styleFlintHelp` `:221`/`:224`, `fix_step_1`…`fix_tail`
`:1038`/`:1044`, `photosCap` `:394`; `ui_time.h:54-60`, `ui_time.c` `:664` (`prv_draw_sync_icon`), `:465-467`,
`:803-809`, `:837-841`, `:1145-1147`, `:1156-1160`. Restano giusti i `page.js:NN` delle righe **7** (`:138`),
**11** (`:128-130`) e **15** (`:133-134`) e tutti i `page.html:NN`. ⚠️ I **`settings.h:NN`** delle righe **7, 10, 11, 15 e 22b**
erano invece **vecchi** (quattro slittati di 4 righe con il commento sopra `GalLayout` di S14/D136; quello di `GalFont` non
tornava già prima: il 19/09 della riga 10 erano stati ricontrollati solo i `page.js:NN`) e sono stati riportati a oggi il
**20/09/2026** (`/code-review`, `grep -n`): `GalFont` `:19-20`, `GalDigitStyle` `:23`, `GalClockMode`/`GalLeadingZero` `:24-25`, `GalOrder` `:28`, `lang` `:51`,
`gal_font_strip()` `:57-62`.
...
(`page.js:535-548`: «Usa questa foto / Non aggiungere» con l'editor aperto, «Salva / Esci senza salvare»
altrimenti).
...
| 4 | Le foto si **scelgono, si ritagliano e si salvano** dal telefono, nella pagina delle impostazioni | `src/pkjs/config/page.html:26-27` (`#file` + etichetta `#add`, chiave `add_photo` «Aggiungi foto», **in cima alla pagina** da UX-2/D80), `:39-57` (editor `#editor`: cornice `#crop` a `:42`, canvas dell'anteprima `#preview` a `:45`), `:106` (`#save`). ⚠️ **«Aggiungi all'album» non esiste più** (UX-3/**D107**, che porta la conferma del ritaglio nel footer): la conferma del ritaglio è `btn_add_ok` = «**Usa questa foto**», scritta **nel footer** da `footerLabels()` (`page.js:539`), mentre la vecchia coppia `#addRow` (`page.html:47`) resta `display:none` con i listener intatti (**D119**); «Salva» è `btn_save` (`page.js:543`). `README.md` §«Come si usa» |
| 5 | L'orologio ne tiene **fino a 12** | `src/pkjs/config/page_core.js:8` (`MAX_SLOTS = 12`); `src/pkjs/config/page.js:394` (`photosCap`: chiavi `photos_cap` «{0} di {1} foto» e `photos_cap_empty`); `docs/design/galleria.md` §2 D8 (12 slot in persist) |
...
| 7 | …in ordine o in modo casuale — ⚠️ **non più dichiarata** nel testo pubblicato (come la riga 6) | `src/c/settings.h:28` (`enum GalOrder`: sequenziale/casuale); default sequenziale in `src/c/settings.c:25`; etichette in `src/pkjs/config/page.js:138` (`o.order`, chiavi `opt_order_seq` «come l'elenco» / `opt_order_random` «a caso») |
...
| 10 | **Sei font** | `src/c/settings.h:19-20` (`enum GalFont` … `GAL_FONT_COUNT = 6`: Anton, Bebas, Barlow, LECO, Francois One, Staatliches); etichette in `src/pkjs/config/page.js:126-127` (`o.font`); `resources/fonts/README.md`. ⚠️ **da S14/D137** (19/09/2026) la riga «Font» è tornata «etichetta + tendina»: le frecce `#fontPrev`/`#fontNext` di UX-2/D87 **non esistono più** (`grep -rn fontPrev src/pkjs/config/` e `grep -rn cycleFont src/pkjs/config/` non trovano nulla), la tendina resta la sola via. ⚠️ uno dei sei, `GAL_FONT_LECO`, **non è una strip bitmap** (`src/c/settings.h:57-62`: `gal_font_strip()` ritorna −1 per LECO) ed è l'unico che **non** funziona in «Ora grande» (`page.js:206` disabilita l'option con `layout === 1`; con «Ora in basso» di S14/D136 va, perché è il layout A specchiato): da **S14/D141** la chiave `opt_font_leco` lo dice al contrario, «**Font di sistema (tranne Ora grande)**» (en «System font (except Big clock)»), invece del «(solo Ora in alto)» di prima, che con tre disposizioni sarebbe falso. Il testo inglese lo conta dentro «Big bitmap clock» (scelta consapevole, il conteggio resta quello dell'enum) |
| 11 | **Quattro stili** di cifre: pieno, solo contorno, due con ombra 3D | `src/c/settings.h:23` (`enum GalDigitStyle`); `docs/design/galleria.md` §2 D21; etichette in `src/pkjs/config/page.js:128-130` (`o.digit_style`): da UX-1 sono «**pieno**», «**solo contorno**», «**contorno con ombra**», «**pieno con ombra**» (la parola «trasparente» è uscita dalla pagina; nel C l'enum resta `GAL_STYLE_OUTLINE`/`GAL_STYLE_OUTLINE_3D`) |
| 12 | Gli stili con contorno/ombra **sono pensati per il Pebble Time 2** e **sul Pebble 2 Duo i due 3D valgono come i piatti** | `docs/design/galleria.md` §2 D26 (su flint niente ombra: 2 → 1, 3 → 0); `src/pkjs/config/page.js:192-202` (`UNAVAIL` + `applyUnavailable()`, che sostituisce `NO_3D` da UX-2/D86) e `:211-212` (le due chiamate): su flint le due option 3D — e i due colori giallo/blu — sono `disabled` **e `hidden`**, con il testo `opt_style_no_flint` = «**{0} (non sul Duo)**»; `src/pkjs/config/page.html:66` (`#s_style_hint`, chiave `style_hint` «Con questo stile si leggono meglio Francois One e Staatliches.», mostrato da `page.js:221` **solo con stile 1 o 2 e font ≤ 2**, cioè Anton/Bebas/Barlow — D98) e `page.html:67` (`#styleFlintHelp`, chiave `style_flint_help`, mostrato da `page.js:224` solo su flint con lo stile 1: R13, l'avviso «contorno sottile» sta **solo nella pagina**, non nella descrizione); `PIANO.md` §7 (anello 1 px al limite su flint, O11 non fatto) |
...
| 14 | **Tre disposizioni**: l'**ora** su un terzo di schermo, **in alto o in basso**, con **passi, batteria e data**, oppure sull'intero schermo da sola (**S14/D136**, 19/09/2026) | `docs/design/galleria.md` §3.1 (da S14 anche il wireframe «Ora in basso», `layout = 2`) e §3.2; §2 D13 (B = solo cifre in v1); `src/c/settings.h:16` (`enum GalLayout`: `GAL_LAYOUT_A` 0, `GAL_LAYOUT_B` 1, **`GAL_LAYOUT_A_BOTTOM` 2** = A specchiato, riga info sopra e cifre a filo del fondo), `GalInfoRowBits`; etichette in `src/pkjs/config/page.js:125` (`o.layout`, tre voci **nell'ordine 0, 2, 1**: «**Ora in alto, info sotto**», «**Ora in basso, info sopra**», «**Ora grande, senza info**»). La descrizione (§2 = `store/description.txt`, riga 3) dice «**Three layouts: time on top or at the bottom, with steps, battery and date, or time only**» **dal 20/09/2026**, mandata con il `PATCH` di `PUBLISH.md` §0.1 (non `pebble publish`; §3.S14) |
| 15 | **12 o 24 ore**, con o senza **zero iniziale** | `src/c/settings.h:24-25` (`GalClockMode`, `GalLeadingZero`, valore AUTO = come l'orologio); default AUTO in `src/c/settings.c:20-21`; etichette in `src/pkjs/config/page.js:133-134` (`o.clock_mode`/`o.leading_zero`: da UX-2 — voce **U-11** di `docs/design/galleria-s13-ux-casual.md` §4, chiavi `opt_clock_auto`/`opt_leading_zero_auto` — «**come l'orologio**», «12 h», «24 h» e «**sì con 24 h, no con 12 h**», «sì (09:05)», «no (9:05)»); le due voci stanno sotto «Altre impostazioni» (`page.html:77-80`, **UX-2/D88**) |
...
| 23 | Se l'avvio diventa lento dopo molte sostituzioni di foto: **Rimuovi (non Aggiorna) e reinstalla** | `src/pkjs/config/page.js:1038` (i quattro passi, chiavi `fix_step_1`…`fix_step_4` di `i18n/messages.json`: «apri l'app Pebble sul telefono», «tocca Galleria nell'elenco delle app», «rimuovi Galleria dall'orologio (non aggiornarla)», «reinstalla Galleria»); `README.md` §«Galleria si avvia lentamente?»; `docs/design/galleria.md` §2 D27 |
| 24 | Le foto **restano sul telefono** e tornano da sole | `src/pkjs/config/page.js:1044` (`T('fix_tail')`, chiave `fix_tail` di `i18n/messages.json`); `docs/design/galleria.md` §5.1 (album in `localStorage`, diff stateless a ogni HELLO) |
...
| 26 | Foto demo **CC0** — ⚠️ «**(Wikimedia Commons)**» era nel testo della 0.1.0 ed è uscito con la riscrittura corta del **05/09/2026**: il testo online dice solo «**Demo photos: CC0.**» (ultima riga di `store/description.txt`, 827 caratteri, 20/09/2026); la provenienza resta in §4, non nel testo pubblicato | `resources/photos/README.md` (licenza verificata con l'API di Commons e con il wikitext della pagina `File:`, provenienza, SHA, CRC32); §4 di questo file; le due CC0 sono nei `.raw` del repo dal 05/09 (CRC32 ricontrollati con `zlib.crc32`) |
...
| 28 | Release notes **della 0.1.0**: il build **Pebble 2 Duo** è controllato **solo in emulatore** e **iOS non è provato** | `PIANO.md` §7 (O11 non fatto: «da vedere sul Pebble 2 Duo vero»); `docs/design/galleria-s8-risultati.md` (ambiente del test: un solo orologio, PT2). ⚠️ **Storia, non stato di oggi**: le note 0.1.0 restano visibili nello store e dicono «iOS is untested», mentre la descrizione dice «tested on Android and iPhone» (riga 20, prova del 06/09). Sulla stessa pagina le due frasi si contraddicono (**dal 20/09/2026** lo store mostra la descrizione di §2, **827 caratteri**, e le note della **1.0.0**; §3.S14 e nota di testa): **le note 1.0.0 e la descrizione sono il testo che vale**, le 0.1.0 sono l'archivio della prima release. Il Pebble 2 Duo resta non provato su orologio vero |
| 29 | Release notes: durante il caricamento un **contatore delle foto** è visibile **sullo schermo** (0.2.0: **icona di sincronizzazione + «k/n»**, senza parole — D32, `src/c/ui_time.c` `prv_draw_sync_icon`, arco 40°–335° + punta su `GPath` statico) | `src/c/ui_time.h:54-60` (contratto di `ui_time_set_sync_progress`: icona di sincronizzazione + «index/count» al posto di passi/icona BT nel layout A, icona + «k/n» nella fascia MM in B); `src/c/ui_time.c:664` (`prv_draw_sync_icon`, arco + punta su `GPath` statico), `:1145-1147` (`snprintf(s_sync_buf, …, "%u/%u")`, `"%u"` senza totale: da S10/D32 **la parola «Foto» non c'è più**), `:837-841` (icona + «k/n» nella riga info del layout A), `:803-809` (**U8/R10**: lo stesso contatore nella fascia dinamica del **layout B**, `MODE_B_SPRITE`), `:465-467` (ridisegno al cambio layout / fine Quick View) e `:1156-1160` (`index` 0 → la fascia ridisegnata lo cancella); `src/c/sync_proto.h:34-35` (R01, 05/09: le foto saltate contano, il contatore arriva a `n/n`) |
| 30 | Nome dello store **«Galleria»** (era «Galleria for Pebble»), versione **1.0.0** (prima release **0.1.0**, poi **0.2.0** e **0.4.0**) | spec S9 §2; `package.json` (`displayName` «Galleria», `version` **1.0.0**, commit `dd628c0` del 20/09/2026), decisione **U7** del 05/09/2026 per la 0.1.0, **D38** (S10) per la 0.2.0, **D42** (S11) per il nome, **D126** (UX-4) per la 0.4.0 al posto della 0.3.0 mai uscita e **S14** per la **1.0.0** del 20/09/2026 (il numero l'ha scelto l'utente, §3.S14); il nome **non si cambia dalla CLI** (`PUBLISH.md` §9) ma con il `PATCH` di `PUBLISH.md` §0/§0.1 (`--form-string "title=Galleria"`) — ⚠️ il nome **online è «Galleria»** senza `PATCH` (verificato sull'API pubblica il 19/09/2026: rinominato dall'utente dalla dashboard fra il 18/09 e il 19/09, §1), mentre il `PATCH` di **§0.1** è stato lanciato **due volte per la sola descrizione**: la sera del **19/09/2026** (794 caratteri) e il **20/09/2026** con la 1.0.0 (**827 caratteri = `store/description.txt`**, §3.S14) |
| 31 | Descrizione e release notes: «**Version 1.0.0**», **non più una beta** | `package.json` → `version` `1.0.0` (commit `dd628c0`, 20/09/2026) e ultima riga di `store/description.txt` («Version 1.0.0, open source (MIT): …»), online dal 20/09/2026 (§3.S14); anche le release notes dicono «Galleria 1.0.0», senza «beta» (`store/release_notes_1.0.0.txt`). Erano beta la 0.1.0 («Beta 0.1.0», decisione **U7**, tag `v0.1.0-beta`), la 0.2.0 («Beta 0.2.0», tag `v0.2.0`) e la 0.4.0 («Beta 0.4.0», tag `v0.4.0`); la 0.3.0 non è mai stata pubblicata (D126): ne resta solo il testo delle note in §3.1 |
...
invece **nella build della 0.1.0** (oggi `src/c/ui_time.c:803-809`, `MODE_B_SPRITE`), quindi la riga «photo
counter» delle release notes vale per **entrambi** i layout: per questo dice «on the screen» e non «in the
info row».
...
unzip -p build/galleria.pbw appinfo.json             # atteso: versionLabel = la versione da pubblicare (1.0.0), companyName Rediro (appinfo.json e' generato)
#   1.0.0 e' quella pubblicata il 20/09/2026 (la 0.4.0 il 18/09/2026); per la prossima release il numero nuovo
python3 store/make_assets.py --check                 # icone e screenshot: --check e basta, NON si rigenerano (D131);
#   dal 19/09/2026 il check copre 8 screenshot (5 emery + 3 flint: il sesto emery tolto) e 3 icone: verde il 19/09 e il 20/09/2026 (prima della 1.0.0)
wc -m store/description.txt store/release_notes_1.0.0.txt   # 828 e 506 (con il newline finale); per la 0.4.0 erano 795 e 697
grep -n "0\.3\.0" README.md                          # deve trovare solo storia datata: la 0.3.0 non e' mai uscita
pebble login --status                                # account: collegato (verificato il 20/09/2026, pre-check della 1.0.0)
```
...
(`PUBLISH.md` §4 e §9). La **1.0.0** (S14) è stata **lanciata il 20/09/2026 alle 00:14 locali** ed è la
release online; la **0.4.0** (D126: si salta la 0.3.0, mai lanciata) era stata **lanciata il 18/09/2026**.
Per la prossima release cambiano solo `--version` e il file delle note:
...
**descrizione** nuova (**827 caratteri** = `store/description.txt`, §2 — **mandata il 20/09/2026** con
la 1.0.0; la sera del **19/09/2026** era andata online quella da 794 caratteri della 0.4.0). Il comando
è lì, con il cookie di sessione; qui basta ricordare che **`title` è obbligatorio in ogni `PATCH`** e
che quindi lo stesso comando fa entrambe le cose: oggi `title=Galleria` non cambia più nulla, ma va
passato ugualmente. ⚠️ Le **release** le ha lanciate l'**orchestratore**, su richiesta dell'utente:
0.4.0 il **18/09/2026** e **1.0.0 il 20/09/2026 alle 00:14 locali**; così i due `PATCH` della
**descrizione**, sera del **19/09/2026** per la 0.4.0 e **20/09/2026** per la 1.0.0, entrambi **HTTP
200**: il giro della **1.0.0** è **completo**, come quello della 0.4.0. Il gate sul telefono
...
| licenza citata nella descrizione | **U1: MIT** | `LICENSE` in radice del repo; ultima riga di `store/description.txt` («Beta 0.1.0. Open source (MIT): github.com/Rediro-MC/galleria-da-polso»), totale 1.493 caratteri (allora; oggi «Version 1.0.0, open source (MIT): github.com/Rediro-MC/galleria-da-polso», 827 caratteri, §2) |
...
| crediti demo | **U3: le due foto CC0 restano** | nessun cambiamento al testo: la descrizione dice «Demo photos: CC0 (Wikimedia Commons)» (allora; oggi «Demo photos: CC0.», §2), gli autori stanno in §4 (le righe lunghe sforerebbero il tetto, §2) |
| firmware minimo nel testo | **U5: SDK 4.33.1 confermato** (si usa l'ultimo SDK, D5 chiusa) | «PebbleOS 4.32 or newer» (il testo di allora; oggi `store/description.txt` dice «PebbleOS 4.32+») resta valido nella sostanza; `docs/design/galleria.md` §2 D5 |
...
| indicatore di sync in layout B | **U8: applicato nella 0.1.0** (R10) | `src/c/ui_time.c:803-809` (`MODE_B_SPRITE`); le release notes dicono «a photo counter on the screen» perché ora vale per entrambi i layout (§3) |
| issue su PebbleOS | **U9: rimandata** → **fatta il 20/09/2026: #2106** | nessun effetto sul listing (`docs/design/galleria-s9-issue-pebbleos.md`, pubblicata come `coredevices/PebbleOS#2106`) |
...
- `--name` vale **solo alla creazione dell'app**: dalla **seconda release in poi** (per Galleria: dalla 0.2.0) nome,
  descrizione, icone e sorgente non si aggiornano più dalla CLI (`PUBLISH.md` §9);
...
> 2. il `PATCH` di `PUBLISH.md` §0/§0.1, che **rinomina l'app in «Galleria»** (D42, mai fatto) e carica la descrizione di allora (794 caratteri, sei lingue, «tested on Android and iPhone», «Beta 0.4.0»).
>
...
>
> **Stato al 19/09/2026**: il punto 1 è **fatto** — la **0.4.0 è stata pubblicata il 18/09/2026** dall'orchestratore, su richiesta dell'utente e **senza** il gate sul telefono. Il punto 2 era **a metà** la mattina: il **nome** è online («Galleria», rinominato **dall'utente dalla dashboard**, verificato sull'API il 19/09), mentre la **descrizione** della 0.4.0 (794 caratteri) **non era ancora stata mandata** — online c'era ancora quella della 0.2.0, 777 caratteri — quindi il `PATCH` di `PUBLISH.md` §0.1 restava da lanciare → **sera del 19/09/2026**: punto 2 **completato** (`PATCH` eseguito, descrizione online = il testo «Beta 0.4.0», 794 caratteri). Il **gate P01–P20 non è stato fatto** e resta utile a release uscita. **D131** valeva per la **release** del 18/09: la stessa notte sono stati aggiunti **7 screenshot in `store/`**, 6 dei quali poi caricati online dall'utente dalla dashboard; il settimo è stato **tolto la sera del 19/09** (punto 4 qui sotto).
>
> **Stato al 20/09/2026**: per la **1.0.0** i due passi sono andati **insieme** — release **pubblicata alle 00:14 locali** dall'orchestratore, su richiesta dell'utente «carica su github e su pebble store» (§3.S14, §6; `PUBLISH.md`, in testa), e `PATCH` di `PUBLISH.md` §0.1 lanciato lo stesso giorno (**HTTP 200**): online **827 caratteri = `store/description.txt`** («Three layouts», «Version 1.0.0», niente «Beta»). Titolo «Galleria» e **8 screenshot** (5 emery + 3 flint) **invariati**, `make_assets.py --check` verde fra i pre-check senza rigenerare nulla (punto 4). Il **gate P01–P20 sul telefono** non c'è stato nemmeno stavolta: la 1.0.0 è uscita dopo la **prova sul PT2 reale via Android** del 19/09 sera (`PIANO.md` §8), e resta utile a release uscita.
...
| U8 | Indicatore di sync in layout B (R10) | **sì, nella 0.1.0** | `src/c/ui_time.c:803-809` (`MODE_B_SPRITE`); §3 e riga 29 di §5 |
| U9 | Issue su PebbleOS (avvio lento) | **rimandata** → **fatta il 20/09/2026: #2106** | nessun effetto sul listing; bozza pubblicata come `coredevices/PebbleOS#2106`, testo in `docs/design/galleria-s9-issue-pebbleos.md` |

Restano solo cose **non decidibili da qui** (il punto 3 si e' chiuso per un'altra strada):

1. **Limiti reali dei campi dello store Core**: ignoti (nessun controllo nel tool, doc 404). La descrizione in
   vigore sta a **827 caratteri**, **oltre** gli 800 chiesti dall'utente per la **0.4.0** (il cui testo ne usava
   794): lo store non ha un tetto noto e ha accettato il `PATCH` del 20/09/2026 (HTTP 200, `PUBLISH.md` §0.1), e
   si resta comunque sotto il tetto prudenziale di 1.500 e i 1.600 dichiarati dalla documentazione Rebble; la
   stesura lunga del 05/09 stava a 1.493.
...
4. **Asset dello store**: rigenerati il 05/09 alle 17:18 dagli screenshot del gate S9-prep
   ...
   **Nota del 20/09/2026**: la **1.0.0** è uscita con gli **stessi 8 screenshot e le stesse 3 icone**, `make_assets.py
   --check` verde fra i pre-check (§6) e **nessuna rigenerazione** («Stato al 20/09/2026» in testa a questa sezione).
